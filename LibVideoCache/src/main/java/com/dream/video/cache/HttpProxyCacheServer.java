...
import com.dream.video.cache.file.Md5FileNameGenerator;
import com.dream.video.cache.file.TotalCountLruDiskUsage;
import com.dream.video.cache.file.TotalSizeLruDiskUsage;
...
import static com.dream.video.cache.Preconditions.checkAllNotNull;
...
public class HttpProxyCacheServer {
    ...
    private HttpProxyCacheServer(Config config) {
        ...
        try {
            ...
            LogUtils.iTag(HttpProxyCacheServer.class.getName(),"Proxy cache server started. Is it alive? " + isAlive());
    ...
    public String getProxyUrl(String url, boolean allowCachedFileUri) {
        ...
        if (allowCachedFileUri && isCached(cacheFile)) {
    ...
    public void registerCacheListener(CacheListener cacheListener, String url) {
        ...
        synchronized (clientsLock) {
            try {
                ...
                LogUtils.wTag(HttpProxyCacheServer.class.getName(),"Error registering cache listener", e);
            }
        }
    }
    ...
    public boolean isCached(File file) {
        checkNotNull(file, "Url can't be null!");
        return file.exists();
    }

    public void shutdown() {
        LogUtils.iTag(HttpProxyCacheServer.class.getName(),"Shutdown proxy server");
    ...
    private void touchFileSafely(File cacheFile) {
        try {
            ...
            LogUtils.eTag(HttpProxyCacheServer.class.getName(),"Error touching file " + cacheFile, e);
        }
    }
    ...
    private void waitForRequest() {
        try {
            while (!Thread.currentThread().isInterrupted()) {
                ...
                LogUtils.iTag(HttpProxyCacheServer.class.getName(),"Accept new socket " + socket);
    ...
    private void processSocket(Socket socket) {
        try {
            ...
            LogUtils.iTag(HttpProxyCacheServer.class.getName(),"Request to cache proxy:" + request);
            ...
            LogUtils.iTag(HttpProxyCacheServer.class.getName(),"Closing socket… Socket is closed by client.");
        } catch (ProxyCacheException | IOException e) {
            onError(new ProxyCacheException("Error processing request", e));
        } finally {
            releaseSocket(socket);
            LogUtils.iTag(HttpProxyCacheServer.class.getName(),"Opened connections: " + getClientsCount());
        }
    }
    ...
    private void closeSocketInput(Socket socket) {
        try {
            ...
            // There is no way to determine that client closed connection http://stackoverflow.com/a/10241044/999458
            // So just to prevent log flooding don't log stacktrace
            LogUtils.iTag(HttpProxyCacheServer.class.getName(),"Releasing input stream… Socket is closed by client.");
        } catch (IOException e) {
    ...
    private void closeSocketOutput(Socket socket) {
        try {
            ...
            LogUtils.wTag(HttpProxyCacheServer.class.getName(),"Failed to close socket on proxy side: {}. It seems client have already closed connection.", e.getMessage());
        }
    }
    ...
    private void onError(Throwable e) {
        LogUtils.eTag(HttpProxyCacheServer.class.getName(),"HttpProxyCacheServer error", e);
    }
