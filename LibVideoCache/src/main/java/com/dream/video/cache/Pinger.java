...
class Pinger {
    ...
    boolean ping(int maxAttempts, int startTimeout) {
        ...
        while (attempts < maxAttempts) {
            try {
                ...
                LogUtils.wTag(Pinger.class.getName(),"Error pinging server (attempt: " + attempts + ", timeout: " + timeout + "). ");
            } catch (InterruptedException | ExecutionException e) {
                LogUtils.eTag(Pinger.class.getName(),"Error pinging server due to unexpected error", e);
            }
            attempts++;
            timeout *= 2;
        }
        ...
        LogUtils.eTag(Pinger.class.getName(),error);
        return false;
    }
    ...
    private boolean pingServer() throws ProxyCacheException {
        ...
        try {
            ...
            LogUtils.iTag(Pinger.class.getName(),"Ping response: `" + new String(response) + "`, pinged? " + pingOk);
            return pingOk;
        } catch (ProxyCacheException e) {
            LogUtils.eTag(Pinger.class.getName(),"Error reading ping response", e);
            return false;
        } finally {
            source.close();
        }
