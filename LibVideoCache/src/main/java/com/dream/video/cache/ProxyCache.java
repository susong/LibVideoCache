...
class ProxyCache {
    ...
    public void shutdown() {
        synchronized (stopLock) {
            LogUtils.iTag(ProxyCache.class.getName(),"Shutdown proxy for " + source);
            try {
                stopped = true;
    ...
    protected final void onError(final Throwable e) {
        ...
        if (interruption) {
            LogUtils.iTag(ProxyCache.class.getName(), "ProxyCache is interrupted");
        } else {
            LogUtils.iTag(ProxyCache.class.getName(), "ProxyCache error" + e);
        }
    }
