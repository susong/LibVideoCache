package com.dream.video.cache.file;
        ...
        for (File file : files) {
            ...
            if (!accepted) {
                ...
                if (deleted) {
                    totalCount--;
                    totalSize -= fileSize;
                    LogUtils.iTag(LruDiskUsage.class.getName(),"Cache file " + file + " is deleted because it exceeds cache limit");
                } else {
                    LogUtils.iTag(LruDiskUsage.class.getName(),"Error deleting file " + file + " for trimming cache");
                }
            }
        }
    }
